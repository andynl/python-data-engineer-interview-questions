...
cur.execute(''' EXPLAIN SELECT customer.firstname, SUM(boughtitem.price) FROM BoughtItem as boughtitem
INNER JOIN Customer as customer ON (customer.id = boughtitem.customerid)
GROUP BY customer.firstname ''')

print(cur.fetchall())

cur.execute(''' EXPLAIN QUERY PLAN SELECT customer.firstname, item.title, item.price, boughtitem.price FROM BoughtItem as boughtitem
INNER JOIN Customer as customer ON (customer.id = boughtitem.customerid)
INNER JOIN Item as item ON (item.id = boughtitem.itemid) ''')
